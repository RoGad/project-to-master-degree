...
```
наблюдение (60x80 RGB) ──► RSSM posterior ──► текущее латентное состояние
                                                      │
                                  N=150 случайных последовательностей действий
                                                      ▼
                              воображаемые rollout'ы (prior, H=10 шагов)
                                                      ▼
                                     декодер ──► будущие кадры 64x64
                                                      ▼
                        CLIP: softmax( sim(кадр, goal), sim(кадр, negative) )
                                                      ▼
                       лучший кандидат ──► ПЕРВОЕ действие ──► среда ──► replan
```

Ключевое требование задания соблюдено: VLM оценивает **будущие воображаемые кадры** rollout-а, а не текущее наблюдение.
